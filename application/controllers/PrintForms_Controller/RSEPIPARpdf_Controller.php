...
class RSEPIPARpdf_Controller extends CI_Controller
{
    public function RSEPIPARform()
    {
        ...
        if($GeneralReport == "1"){
            $RegSPIDatas = $this->Fpdf_Model->regspi_itemPAR($Property,$PropertyYear);
        }
        if($GeneralReport == "2"){
            $RegSPIDatas = $this->Fpdf_Model->regspi_individualPAR($Property,$PropertyYear,$IndividualReport);
        }
        ...
        $pdf = new PDF();
            ...
            foreach($RegSPIDatas as $Data){
                ...
                $pdf->SetFont('times', '', 8);
                $pdf->SetXY($x+15, $y + 14); 
                $pdf->multicell(18, 6, $Data->par_no, '','C'); 
                $pdf->SetXY($x+33, $y + 14); 
